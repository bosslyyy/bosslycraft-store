...
export function Hero(){
 ...
 return <section className="hero-section grid-bg relative overflow-hidden pt-20"><div className="hero-aura absolute left-1/2 top-32 h-96 w-96 -translate-x-1/2 blur-[120px]"/><div className="relative mx-auto grid max-w-6xl items-center gap-4 px-6 pb-28 pt-14 md:grid-cols-[.9fr_1.1fr] md:pb-36 md:pt-20">
   ...
   <div className="funding-panel pixel-panel max-w-xl p-5 md:col-span-2 md:mt-6"><div className="mb-3 flex items-end justify-between"><div><p className="text-xs uppercase tracking-widest text-slate-400">Hosting · este mes</p><p className="minecraft-heading mt-1 text-lg font-black uppercase">Mantengamos el mundo vivo</p></div><span className="font-mono text-sm text-cyan-200">${(funding.raisedCents/100).toFixed(2)} / ${(funding.goalCents/100).toFixed(2)}</span></div><div className="funding-track h-5 border-2 p-0.5"><div className="funding-fill h-full transition-all duration-700" style={{width:`${pct}%`}}/></div><p className="mt-3 flex items-center gap-2 text-xs text-slate-400"><Server className="h-3.5 w-3.5 text-violet-300"/>{pct}% de la meta mensual</p></div>
 </div></section>
}
